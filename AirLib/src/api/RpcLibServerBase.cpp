    ...
    : api_provider_(api_provider)
{
    ...
    pimpl_->server.bind("simGetObjectTwist", [&](const std::string& object_name) -> RpcLibAdapatorsBase::Twist {
        const auto& twist = getWorldSimApi()->getObjectTwist(object_name);
        return RpcLibAdapatorsBase::Twist(twist);
    });
